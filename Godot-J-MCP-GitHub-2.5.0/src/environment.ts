...
export async function checkExecutable(command: string): Promise<{ ok: boolean; version?: string; error?: string }> {
  try {
    const r = await runProcess(command, ['--version'], { timeoutMs: 15000 });
