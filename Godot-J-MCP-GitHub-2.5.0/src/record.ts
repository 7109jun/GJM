...
export async function clearFrames(projectRoot: string): Promise<void> {
  await fs.rm(safeProjectPath(projectRoot, '.gjm_frames'), { recursive: true, force: true });
}

export async function locateFrames(projectRoot: string): Promise<string[]> {
  const dir = safeProjectPath(projectRoot, '.gjm_frames');
  const names = await fs.readdir(dir).catch(() => []);
  return names
    .filter((name: string) => /^frame_\d{6}\.png$/i.test(name))
    .sort()
    .map((name: string) => path.join(dir, name));
}

export async function exportMp4(projectRoot: string, outputName: string, fps: number) {
  const frames = await locateFrames(projectRoot);
  if (frames.length === 0) return { ok: false, message: 'No captured PNG frames found.' };
...
export async function locateCheckpoints(projectRoot: string): Promise<string[]> {
  const dir = safeProjectPath(projectRoot, '.gjm_frames');
  const names = await fs.readdir(dir).catch(() => []);
  return names.filter((name: string) => /^checkpoint_\d{3}_.+\.png$/i.test(name)).sort().map((name: string) => path.join(dir, name));
}
