function actionScript(path, content) {
    return { type: 'script', path, content };
}
function scene(path, root, nodes = [], subresources = [], connections = []) {
    return { type: 'scene', path, root, nodes, subresources, connections };
}
function platformer3() {
    return [
...
export function expandTemplates(names, godotMajor) {
    const actions = [];
    for (const name of names) {
        switch (name) {
            case 'platformer_2d':
                actions.push(...platformer3());
                break;
            case 'topdown_2d':
                actions.push(...topdown3());
                break;
            case 'third_person_3d':
                actions.push(...(godotMajor >= 4 ? thirdPerson4() : topdown3()));
                break;
            case 'first_person_3d':
                actions.push(...(godotMajor >= 4 ? firstPerson4() : topdown3()));
                break;
            case 'ui_menu':
                actions.push(...uiMenu());
                break;
            case 'save_system':
                actions.push(...saveSystem());
                break;
            case 'audio_manager':
                actions.push(...audioManager());
                break;
