...
export async function collectContext(projectRoot, options = {}) {
    ...
    if (includeFiles) {
        ...
        files = listed.slice(0, maxFiles).map((item) => {
            if (!includeContent || item.type !== 'file')
                return item;
            try {
                const abs = safeProjectPath(root, item.path);
                const bytes = Number(item.size ?? 0);
                if (bytes > 200_000)
                    return { ...item, contentOmitted: 'file_too_large' };
                return { ...item, content: requireRead(abs) };
            }
            catch {
                return { ...item, contentOmitted: 'unreadable' };
            }
        });
    }
    let project = null;
