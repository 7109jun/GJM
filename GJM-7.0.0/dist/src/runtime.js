...
function gdString(value) { return JSON.stringify(value); }
function emitBridgeScript(mainScene) {
    return `extends Node
...
export async function readRuntime(root) {
    try {
        return JSON.parse(await fs.readFile(safeProjectPath(root, STATE), 'utf8'));
    }
    catch {
        return null;
    }
}
export async function startRuntime(root) {
    ...
    const procPromise = runGodot(['--path', root, major >= 4 ? '--scene' : SCENE, ...(major >= 4 ? [SCENE] : [])], root, 60000, { GJM_RUNTIME_PORT: String(portHint), GJM_RUNTIME_TOKEN: token });
    void procPromise.catch(() => undefined);
    const state = { id: randomUUID(), token, port: portHint, startedAt: new Date().toISOString(), projectRoot: root };
    ...
    if (!foundPort) {
        ...
        throw new Error('GJM runtime bridge failed to start.');
    }
    state.port = foundPort;
    await writeState(root, state);
    return state;
}
export async function runtimeCommand(root, request) {
...
export async function stopRuntime(root) {
    const state = await readRuntime(root);
    if (!state)
        return;
    try {
        await runtimeCommand(root, { op: 'quit' });
    }
    catch { }
    await new Promise(r => setTimeout(r, 100));
