...
async function workspaceList(root, relative = '.', recursive = false) {
    ...
    if ((await statKind(base)) !== 'directory')
        throw new Error(`Directory not found: ${relative}`);
    await visit(base, relative === '.' ? '.' : relative);
    return out;
}
async function workspaceSearch(root, query, relative = '.', maxResults = 100) {
    if (!query)
        throw new Error('query is required.');
    const files = await workspaceList(root, relative, true);
    const matches = [];
    for (const entry of files) {
        if (entry.type !== 'file' || (entry.size ?? 0) > 2_000_000)
            continue;
        ...
        let text = '';
        try {
            text = await fs.readFile(safeProjectPath(root, entry.path), 'utf8');
        }
        catch {
            continue;
        }
...
async function projectSettings(root) {
    ...
    for (const line of raw.split(/\r?\n/)) {
        ...
        if (m && section)
            result[section][m[1].trim()] = m[2].trim();
    }
    return result;
}
async function git(root, args) {
    ...
    if (!allowed.has(args[0]))
        throw new Error(`Unsupported git operation: ${args[0]}`);
    if (args.includes('-C'))
        throw new Error('Do not pass git -C. GJM supplies the project root.');
    const result = await execFileAsync('git', args, { cwd: root, maxBuffer: 10 * 1024 * 1024 });
    return { code: 0, stdout: result.stdout, stderr: result.stderr };
}
export async function workspaceAction(root, op, payload) {
    switch (op) {
        case 'list':
...
export async function projectAction(root, op, payload) {
    switch (op) {
        case 'info':
...
export async function gitAction(root, args) { return git(root, args); }
