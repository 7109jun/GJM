...
function gdQuote(v) { return JSON.stringify(v); }
function gdValue(v) {
...
function blockForScene(scene, ops, index) {
    ...
    if (createOps.length > 0) {
        if (createOps.length > 1)
            throw new Error(`Multiple create operations for scene: ${scene}`);
        const c = createOps[0];
        ...
        if (op.op === 'add_node') {
            ...
            if (op.properties)
                for (const [k, v] of Object.entries(op.properties))
                    out.push(`            node_${p}.set(${gdQuote(k)}, ${gdValue(v)})`);
            if (op.script)
    ...
    if (!createOnly)
        out.push(`    else: results.append({"scene": scene_path, "error":"scene_load_failed"})`);
    return out;
}
function makeScript(ops, resultFile) {
...
export function buildSceneTaskScript(ops, resultFile) { return makeScript(ops, resultFile); }
export async function sceneOperations(projectRoot, ops, timeoutMs = 120000) {
    if (!Array.isArray(ops) || ops.length === 0)
        throw new Error('At least one scene operation is required.');
    if (ops.length > 100)
        throw new Error('A maximum of 100 scene operations is allowed per call.');
    ...
    try {
        ...
        let result = null;
        try {
            result = JSON.parse(await fs.readFile(safeProjectPath(root, relResult), 'utf8'));
        }
        catch { }
...
//# sourceMappingURL=scene-ops.js.map