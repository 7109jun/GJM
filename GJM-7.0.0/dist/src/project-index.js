...
async function readText(root, file, maxBytes) {
    try {
...
function extractExtResources(text) {
...
function extractNodes(text) {
    ...
    for (const m of text.matchAll(re))
        out.push({ name: m[1], type: m[2], ...(m[3] ? { parent: m[3] } : {}) });
    return out;
}
...
export async function buildProjectIndex(projectRoot, options = {}) {
    ...
    const listed = await workspaceAction(root, 'list', { path: '.', recursive: true });
    ...
    for (const item of files) {
        ...
        if (file.endsWith('.tscn')) {
            ...
            const body = await readText(root, file, maxSourceBytes);
            if (body !== undefined)
                scripts.push({ path: file, classes: extractClasses(body), functions: extractFunctions(body), ...(includeSource ? { source: body } : {}) });
        }
