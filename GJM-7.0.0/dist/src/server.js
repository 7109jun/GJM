...
export function createServer() {
    const server = new McpServer({ name: 'Godot-J-MCP', version: '7.0.0' }, {
        instructions: 'GJM is a Godot development and verification MCP server. Prefer gjm_execute for the full AI workflow: apply godot.make.json, validate Godot, request or apply Behavior.json, run the behavior, evaluate assertions, export MP4, and return artifacts. Use gjm_diagnose and gjm_prepare_fix after failures. All project paths must remain inside projectRoot.'
    });
    server.registerResource('godot-make-schema', 'gjm://schema/godot.make.json', {
    ...
    server.registerResource('behavior-schema', 'gjm://schema/Behavior.json', {
    ...
    server.registerResource('gjm-result-schema', 'gjm://schema/result.json', {
    ...
    server.registerPrompt('gjm-build-game', {
    ...
    server.registerPrompt('gjm-test-game', {
    ...
    server.registerPrompt('gjm-agent', {
    ...
    server.registerPrompt('gjm-anything', {
...
${request}`
    ...
    server.registerPrompt('gjm-debug-and-fix', {
    ...
    server.registerResource('gjm-capabilities', 'gjm://capabilities', { title: 'GJM capabilities', description: 'Machine-readable overview of GJM AI-first capabilities.', mimeType: 'application/json' }, async (uri) => ({ contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify({
                    version: '7.0.0',
    ...
    server.registerTool('gjm_next', {
        ...
        try {
            ...
            const resultObject = result !== null && typeof result === 'object' ? result : undefined;
            if (!session)
                return text({ ok: false, code: GJM_CODES.ENVIRONMENT, next: { action: 'gjm_session_start', reason: 'No session exists.' } });
            if (session.status === 'completed')
                return text({ ok: true, code: GJM_CODES.OK, next: { action: 'gjm_read_result', reason: 'Session completed.' }, session, result });
            if (session.status !== 'active')
                return text({ ok: true, code: GJM_CODES.OK, next: { action: 'gjm_session_start', reason: 'Current session is not active.' }, session, result });
            const code = typeof resultObject?.code === 'string' ? resultObject.code : undefined;
            if (!result)
                return text({ ok: true, code: GJM_CODES.OK, next: { action: 'gjm_execute', reason: 'Start the first workflow attempt.' }, session });
            if (code === GJM_CODES.BEHAVIOR_REQUIRED)
                return text({ ok: true, code: GJM_CODES.OK, next: { action: 'gjm_execute', reason: 'Godot validation passed; provide Behavior.json and execute again.', requires: 'Behavior.json' }, session, result });
            if (typeof code === 'string' && code.startsWith('GJM-E2'))
                return text({ ok: true, code: GJM_CODES.OK, next: { action: 'gjm_diagnose', reason: 'Godot/runtime failure needs AI diagnosis.' }, session, result });
            if (code === GJM_CODES.ASSERTION_FAILED || code === GJM_CODES.BEHAVIOR_RUNTIME)
                return text({ ok: true, code: GJM_CODES.OK, next: { action: 'gjm_diagnose', reason: 'Behavior/test failure needs diagnosis or a revised Behavior.json.' }, session, result });
    ...
    server.registerTool('gjm_pipeline', {
        ...
        try {
            ...
            if (validation.code !== 0) {
                ...
                if (session)
                    await updateSession(root, { status: 'failed', phase: result.phase, lastCode: result.code });
            ...
            await writeRunResult(root, result);
            if (session)
                await updateSession(root, { status: video.ok ? 'completed' : 'failed', phase: result.phase, lastCode: result.code });
    ...
    server.registerTool('gjm_runtime_command', {
        ...
        try {
            ...
            if (!runtimeState)
                throw new Error('No active GJM runtime.');
            if (runtimeId !== undefined && runtimeId !== runtimeState.id)
                throw new Error('Unknown runtimeId.');
    ...
    server.registerTool('gjm_project', {
        ...
        try {
            const root = normalizeProjectRoot(projectRoot);
            if (op === 'export' && (!preset || !output))
                throw new Error('export requires preset and output.');
    ...
    server.registerTool('gjm_assets', {
        ...
        try {
            const root = normalizeProjectRoot(projectRoot);
            if (op === 'list')
                return text({ ok: true, code: GJM_CODES.OK, assets: await listAssets(root) });
    ...
    server.registerTool('gjm_scene_operations', {
        ...
        try {
            const result = await sceneOperations(projectRoot, operations, timeoutMs);
            return text(result, !result.ok);
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
    ...
    server.registerTool('gjm_godot_script', {
        ...
        try {
            return text(await runGodotScriptTask({ projectRoot, script, args, timeoutMs, resultFile }));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_plan', {
        ...
        try {
            ...
            return text(validatePlan(steps));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_do_anything', {
        ...
        try {
            return text(await doAnything({ projectRoot, steps: steps, snapshot, stopOnError, rollbackOnError }));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_universal', {
        ...
        try {
            ...
            switch (operation) {
                ...
                case 'assets_import': {
                    const r = await importAssets(root, Number(p.timeoutMs ?? 120000));
                    return text({ ok: r.code === 0, code: r.code === 0 ? GJM_CODES.OK : gjmErrorCode(r), result: r });
                }
                case 'template': {
                    const name = String(p.name ?? '');
                    const major = await getGodotMajor();
                    const actions = expandTemplatesShim([name], major);
                    const make = { format: 'godot.make', version: 1, actions };
                    return text({ ok: true, code: GJM_CODES.OK, template: name, make });
                }
    ...
    server.registerTool('gjm_context', {
        ...
        try {
            return text(await collectContext(projectRoot, { includeFiles, includeContent, maxFiles }));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_project_index', {
        ...
        try {
            return text(await buildProjectIndex(projectRoot, { maxFiles, includeSource, maxSourceBytes }));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_transaction', {
        ...
        try {
            return text(await runTransaction({ projectRoot, steps: steps, keepSnapshot }));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_editor_plugin', {
        ...
        try {
            return text(await createEditorPlugin(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_profile', {
        ...
        try {
            return text(await profileProject(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_network_test', {
        ...
        try {
            return text(await networkTest(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_generate_wav', {
        ...
        try {
            return text(await generateWavTone(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_generate_svg', {
        ...
        try {
            return text(await generateSvg(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_animation', {
        ...
        try {
            return text(await generateAnimation(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_model', {
        ...
        try {
            return text(await generateModel(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_rig', {
        ...
        try {
            return text(await generateRig(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_shader', {
        ...
        try {
            return text(await createShader(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_audio', {
        ...
        try {
            return text(await importAudio(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_input', {
        ...
        try {
            return text(await configureInput(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_network', {
        ...
        try {
            return text(await scaffoldNetwork(normalizeProjectRoot(projectRoot), options));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_autoplay', {
        ...
        try {
            return text(await autoplay(normalizeProjectRoot(projectRoot), { behavior: typeof behavior === 'string' ? behavior : JSON.stringify(behavior), runs, stopOnFailure }));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_studio', {
        ...
        try {
            return text(await createStudioWorkspace(normalizeProjectRoot(projectRoot), { ...spec, snapshot }));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_studio_plan', {
        ...
        try {
            const root = normalizeProjectRoot(projectRoot);
            const job = startJob('studio', root, async () => runStudioPlan(root, spec));
            return text(job);
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
    ...
    server.registerTool('gjm_job_start', {
        ...
        try {
            const root = normalizeProjectRoot(projectRoot);
            const p = (payload && typeof payload === 'object') ? payload : {};
            const job = startJob(type, root, async () => {
                if (type === 'validate')
                    return validateGodotProject(root);
                if (type === 'execute')
                    return runWorkflow({ projectRoot: root, makeFile: typeof p.makeFile === 'string' ? p.makeFile : undefined, makeContent: typeof p.makeContent === 'string' ? p.makeContent : undefined, behaviorFile: typeof p.behaviorFile === 'string' ? p.behaviorFile : undefined, behaviorContent: typeof p.behaviorContent === 'string' ? p.behaviorContent : undefined, maxAttempts: Number(p.maxAttempts ?? 5), snapshot: p.snapshot !== false });
                if (type === 'autoplay')
                    return autoplay(root, { behavior: typeof p.behavior === 'string' ? p.behavior : JSON.stringify(p.behavior ?? null), runs: Number(p.runs ?? 3), stopOnFailure: p.stopOnFailure !== false });
                return runStudioPlan(root, p.spec && typeof p.spec === 'object' ? p.spec : p);
            });
            return text(job);
        }
        catch (error) {
    ...
    server.registerTool('gjm_parallel', {
        ...
        try {
            const root = normalizeProjectRoot(projectRoot);
            const results = await performParallel(parallelJobs, maxConcurrency, async (item) => { const { doAnything } = await import('./ai-anything.js'); return doAnything({ projectRoot: root, steps: [{ id: 'parallel', op: item.op, args: item.args ?? {} }], snapshot: false, stopOnError: true, rollbackOnError: false }); });
            return text({ ok: results.every((r) => r.ok), code: results.every((r) => r.ok) ? GJM_CODES.OK : GJM_CODES.ENVIRONMENT, concurrency: maxConcurrency, results });
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    server.registerTool('gjm_dependency_graph', {
        ...
        try {
            return text(await buildDependencyGraph(projectRoot, { maxFiles }));
        }
        catch (error) {
            return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
        }
    ...
    }, async ({ projectRoot, maxFiles }) => { try {
        return text(await auditProject(normalizeProjectRoot(projectRoot), { maxFiles }));
    }
    catch (error) {
        return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
    } });
    ...
    }, async ({ projectRoot, ...options }) => { try {
        return text(await generateTexture(normalizeProjectRoot(projectRoot), options));
    }
    catch (error) {
        return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
    } });
    server.registerTool('gjm_animation_edit', {
        ...
        return text(await editAnimation(normalizeProjectRoot(projectRoot), options));
    }
    catch (error) {
        return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
    } });
    server.registerTool('gjm_profile_compare', {
        ...
        return text(await profileCompare(normalizeProjectRoot(projectRoot), options));
    }
    catch (error) {
        return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
    } });
    server.registerTool('gjm_shader_validate', {
        ...
        return text(await shaderValidate(normalizeProjectRoot(projectRoot), options));
    }
    catch (error) {
        return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
    } });
    server.registerTool('gjm_autoplay_fuzz', {
        ...
        return text(await autoplayFuzz(normalizeProjectRoot(projectRoot), options));
    }
    catch (error) {
        return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
    } });
    ...
    }, async ({ projectRoot, actions }) => { try {
        return text(await inputAudit(normalizeProjectRoot(projectRoot), { actions }));
    }
    catch (error) {
        return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
    } });
    server.registerTool('gjm_release_readiness', {
        ...
        inputSchema: z.object({ projectRoot: z.string() })
    }, async ({ projectRoot }) => { try {
        return text(await releaseReadiness(normalizeProjectRoot(projectRoot)));
    }
    catch (error) {
        return text({ ok: false, code: GJM_CODES.ENVIRONMENT, message: errorMessage(error) }, true);
    } });
    server.registerTool('gjm_release', {
        ...
        try {
            const root = normalizeProjectRoot(projectRoot);
            if (op === 'manifest')
                return text({ ok: true, code: GJM_CODES.OK, manifest: await getReleaseManifest(root) });
            if (op === 'package')
                return text(await createReleasePackage(root, output));
