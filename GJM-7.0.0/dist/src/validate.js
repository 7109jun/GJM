...
export function validateMake(value) {
    const root = obj(value, 'godot.make.json');
    if (root.format !== 'godot.make')
        throw new Error('format must be "godot.make".');
    ...
    if (root.description !== undefined)
        str(root.description, 'description');
    root.actions.forEach((raw, i) => {
        ...
        if (type === 'file' || type === 'script' || type === 'shader' || type === 'resource') {
            ...
            if (typeof a.content !== 'string')
                throw new Error(`actions[${i}].content must be a string.`);
            return;
        }
        if (type === 'asset_base64') {
            str(a.path, `actions[${i}].path`);
            if (typeof a.content !== 'string' || !a.content.trim())
                throw new Error(`actions[${i}].content must be a non-empty base64 string.`);
            return;
        }
        if (type === 'copy') {
        ...
        if (type === 'input_action') {
            ...
            if (a.deadzone !== undefined) {
                const d = num(a.deadzone, `actions[${i}].deadzone`);
                if (d < 0 || d > 1)
                    throw new Error(`actions[${i}].deadzone must be 0..1.`);
            }
            if (a.events !== undefined && !Array.isArray(a.events))
                throw new Error(`actions[${i}].events must be an array.`);
            return;
        }
        if (type === 'template') {
            str(a.name, `actions[${i}].name`);
            ...
            if (!allowedTemplates.has(String(a.name)))
                throw new Error(`Unsupported template: ${String(a.name)}`);
            if (a.options !== undefined)
                obj(a.options, `actions[${i}].options`);
            return;
        }
        if (type === 'autoload') {
            str(a.name, `actions[${i}].name`);
        ...
        if (type === 'project_setting') {
            str(a.section, `actions[${i}].section`);
            str(a.key, `actions[${i}].key`);
            if (a.value === undefined)
                throw new Error(`actions[${i}].value is required.`);
            return;
        }
        if (type === 'feature') {
            ...
            if (!allowed.has(String(a.kind)))
                throw new Error(`Unsupported feature kind: ${String(a.kind)}`);
            if (a.scene !== undefined)
                str(a.scene, `actions[${i}].scene`);
            if (a.name !== undefined)
                str(a.name, `actions[${i}].name`);
            if (a.parent !== undefined)
                str(a.parent, `actions[${i}].parent`);
        ...
        if (type === 'godot_script') {
            ...
            if (a.args !== undefined && !Array.isArray(a.args))
                throw new Error(`actions[${i}].args must be an array.`);
            if (a.timeoutMs !== undefined) {
                const t = num(a.timeoutMs, `actions[${i}].timeoutMs`);
                if (t < 1 || t > 600000)
                    throw new Error(`actions[${i}].timeoutMs out of range.`);
            }
            if (a.resultFile !== undefined)
                str(a.resultFile, `actions[${i}].resultFile`);
            return;
        }
        if (type === 'scene_raw') {
            str(a.path, `actions[${i}].path`);
        ...
        if (type === 'scene') {
            ...
            if (a.root !== undefined) {
                ...
                if (r.name !== undefined)
                    str(r.name, `actions[${i}].root.name`);
                if (r.type !== undefined)
                    str(r.type, `actions[${i}].root.type`);
            }
            if (a.nodes !== undefined) {
                if (!Array.isArray(a.nodes))
                    throw new Error(`actions[${i}].nodes must be an array.`);
                for (let j = 0; j < a.nodes.length; j++) {
                    const n = obj(a.nodes[j], `actions[${i}].nodes[${j}]`);
                    str(n.name, `actions[${i}].nodes[${j}].name`);
                    str(n.type, `actions[${i}].nodes[${j}].type`);
                    if (n.parent !== undefined)
                        str(n.parent, `actions[${i}].nodes[${j}].parent`);
                }
            }
            if (a.subresources !== undefined) {
                if (!Array.isArray(a.subresources))
                    throw new Error(`actions[${i}].subresources must be an array.`);
            ...
            if (a.connections !== undefined) {
                if (!Array.isArray(a.connections))
                    throw new Error(`actions[${i}].connections must be an array.`);
...
export function validateBehavior(value) {
    ...
    if (root.format !== 'godot.behavior')
        throw new Error('format must be "godot.behavior".');
    const version = num(root.version, 'version');
    if (!Number.isInteger(version) || version < 1)
        throw new Error('version must be a positive integer.');
    if (!Array.isArray(root.actions))
    ...
    root.actions.forEach((raw, i) => {
        const a = obj(raw, `actions[${i}]`);
        if (typeof a.type !== 'string' || !allowed.has(a.type))
            throw new Error(`Unsupported Behavior action at index ${i}.`);
        if (a.type === 'wait') {
            const v = num(a.ms, `actions[${i}].ms`);
            if (v < 0 || v > 300000)
                throw new Error(`actions[${i}].ms out of range.`);
        }
        if (['key_down', 'key_up', 'key_press'].includes(a.type))
            str(a.key, `actions[${i}].key`);
        if (['mouse_move', 'mouse_down', 'mouse_up', 'click', 'double_click'].includes(a.type)) {
            num(a.x, `actions[${i}].x`);
            num(a.y, `actions[${i}].y`);
        }
        if (a.type === 'click_node')
            str(a.path, `actions[${i}].path`);
        if (a.type === 'key_press' && a.durationMs !== undefined) {
            const v = num(a.durationMs, `actions[${i}].durationMs`);
            if (v < 0 || v > 300000)
                throw new Error(`actions[${i}].durationMs out of range.`);
        }
        if (a.type === 'scroll' && a.deltaX === undefined && a.deltaY === undefined)
            throw new Error(`actions[${i}] scroll needs deltaX or deltaY.`);
        if (a.type === 'inspect_node')
            str(a.path, `actions[${i}].path`);
        if (a.type === 'runtime_command')
            str(a.op, `actions[${i}].op`);
    });
    if (root.assertions !== undefined) {
        ...
        root.assertions.forEach((raw, i) => {
            ...
            if (typeof a.type !== 'string')
                throw new Error(`assertions[${i}].type must be a string.`);
            switch (a.type) {
                case 'frames_min': {
                    const v = num(a.value, `assertions[${i}].value`);
                    if (v < 0 || !Number.isInteger(v))
                        throw new Error(`assertions[${i}].value must be a non-negative integer.`);
                    break;
                }
                case 'checkpoint':
                ...
                case 'exit_code':
                    if (!Number.isInteger(a.value))
                        throw new Error(`assertions[${i}].value must be an integer.`);
                    break;
                case 'node_exists':
                    str(a.path, `assertions[${i}].path`);
                    break;
                case 'node_property':
                    ...
                    if (!(Object.prototype.hasOwnProperty.call(a, 'equals')))
                        throw new Error(`assertions[${i}].equals is required.`);
                    break;
                default:
    ...
    if (root.capture !== undefined) {
        ...
        if (c.fps !== undefined) {
            const v = num(c.fps, 'capture.fps');
            if (v < 1 || v > 120)
                throw new Error('capture.fps must be 1..120.');
        }
        for (const k of ['width', 'height'])
            if (c[k] !== undefined) {
                const v = num(c[k], `capture.${k}`);
                if (v < 1 || v > 7680)
                    throw new Error(`capture.${k} out of range.`);
            }
        if (c.outputName !== undefined)
            str(c.outputName, 'capture.outputName');
    }
    return value;
}
