...
async function files(root, dir, out) {
    for (const e of await fs.readdir(dir, { withFileTypes: true })) {
        if (EXCLUDED.has(e.name))
            continue;
        const abs = path.join(dir, e.name);
        if (e.isDirectory())
            await files(root, abs, out);
        else if (e.isFile() && EXTENSIONS.has(path.extname(e.name).toLowerCase()))
            out.push(path.relative(root, abs).replaceAll(path.sep, '/'));
    }
}
export async function buildDependencyGraph(projectRoot, options = {}) {
    const root = normalizeProjectRoot(projectRoot);
    const all = [];
    await files(root, root, all);
    all.sort();
    const selected = all.slice(0, options.maxFiles ?? 2000);
    const nodes = selected.map((p) => ({ id: p, type: path.extname(p).slice(1) || 'file' }));
    const edges = [];
    const selectedSet = new Set(selected);
    const byBase = new Map();
    for (const p of selected)
        byBase.set(path.basename(p).replace(path.extname(p), ''), p);
    for (const p of selected) {
        ...
        for (const ref of refs) {
            ...
            if (target && selectedSet.has(target) && target !== p)
                edges.push({ from: p, to: target, kind: 'reference' });
        }
        if (p.endsWith('.gd')) {
            const cls = text.match(/^class_name\s+([A-Za-z_][A-Za-z0-9_]*)/m)?.[1];
            const ext = text.match(/^extends\s+(.+)$/m)?.[1]?.trim();
            if (cls)
                nodes.find(n => n.id === p).className = cls;
            if (ext)
                nodes.find(n => n.id === p).extends = ext;
        }
    }
