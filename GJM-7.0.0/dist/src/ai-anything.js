...
import { getGodotMajor, validateGodotProject, runGodot, runGodotProject } from './godot.js';
...
async function scriptPatch(root, args) {
    const rel = String(args.path ?? '');
    if (!rel)
        throw new Error('script_patch requires path.');
    ...
    if (mode === 'replace') {
        ...
        if (!from)
            throw new Error('script_patch replace requires from.');
        ...
        if (!Number.isInteger(line) || line < 1)
            throw new Error('line_replace requires a positive 1-based line.');
        ...
        const line = Number(args.line);
        if (!Number.isInteger(line) || line < 1)
            throw new Error('line_insert requires a positive 1-based line.');
        const lines = content.split(/\r?\n/);
        ...
        const line = Number(args.line);
        if (!Number.isInteger(line) || line < 1)
            throw new Error('line_delete requires a positive 1-based line.');
        const lines = content.split(/\r?\n/);
        if (line > lines.length)
            throw new Error(`Line out of range: ${line}`);
...
async function scenePatch(root, args) {
    const rel = String(args.path ?? '');
    if (!rel.endsWith('.tscn'))
        throw new Error('scene_patch path must end in .tscn');
    ...
    if (mode === 'raw_replace') {
        ...
        const to = String(args.to ?? '');
        if (!from)
            throw new Error('raw_replace requires from.');
...
async function godotCli(root, args) {
    const mode = String(args.mode ?? 'validate');
    const timeoutMs = Number(args.timeoutMs ?? 60000);
    const allowed = [];
    if (mode === 'validate')
        allowed.push(...['--path', root, '--headless', '--editor', '--quit']);
    if (mode === 'run')
        allowed.push(...['--path', root]);
    if (mode === 'scene')
        allowed.push(...['--path', root, '--scene', String(args.scene ?? '')]);
    if (mode === 'check')
        allowed.push(...['--headless', '--path', root, '--check-only']);
    if (mode === 'export') {
    ...
    if (allowed.length === 0)
        throw new Error(`Unsupported godot_cli mode: ${mode}`);
    return runGodot(allowed, root, timeoutMs);
}
function getPathValue(value, expression) {
...
function interpolateDeep(value, context) {
    ...
    if (Array.isArray(value))
        return value.map(v => interpolateDeep(v, context));
...
function testCondition(condition, context) {
    ...
    if (Array.isArray(condition.and))
        return condition.and.every(x => testCondition(x, context));
    if (Array.isArray(condition.or))
        return condition.or.some(x => testCondition(x, context));
    if (typeof condition.not === 'object' && condition.not)
        return !testCondition(condition.not, context);
    const value = condition.path ? getPathValue(context, String(condition.path)) : condition.value;
    if ('exists' in condition)
        return (value !== undefined) === Boolean(condition.exists);
    if ('equals' in condition)
        return value === interpolateDeep(condition.equals, context);
    if ('notEquals' in condition)
        return value !== interpolateDeep(condition.notEquals, context);
    if ('contains' in condition)
        return String(value ?? '').includes(String(interpolateDeep(condition.contains, context)));
    if ('truthy' in condition)
        return Boolean(value) === Boolean(condition.truthy);
    return true;
}
async function ensureProject(root, args) {
    try {
        await fs.access(safeProjectPath(root, 'project.godot'));
        return { created: false, path: 'project.godot' };
    }
    catch { /* create below */ }
...
async function executeStep(root, step) {
    const a = step.args ?? {};
    switch (step.op) {
        ...
        case 'scene_operations': return sceneOperations(root, Array.isArray(a.operations) ? a.operations : [], Number(a.timeoutMs ?? 120000));
        case 'patch_script': return scriptPatch(root, a);
        case 'patch_scene': return scenePatch(root, a);
        case 'set_project_setting': {
            const make = { format: 'godot.make', version: 1, name: 'setting', actions: [{ type: 'project_setting', section: String(a.section), key: String(a.key), value: a.value }] };
        ...
        case 'template': {
            ...
            const make = { format: 'godot.make', version: 1, name, actions };
            if (a.apply === false)
                return { template: name, make };
        ...
        case 'project_index': return (await import('./project-index.js')).buildProjectIndex(root, a);
        case 'transaction': return (await import('./transaction.js')).runTransaction({ projectRoot: root, steps: Array.isArray(a.steps) ? a.steps : [], keepSnapshot: a.keepSnapshot === true });
        case 'dependency_graph': return buildDependencyGraph(root, { maxFiles: Number(a.maxFiles ?? 2000) });
        case 'release': {
            const op = String(a.op ?? 'manifest');
            if (op === 'manifest')
                return getReleaseManifest(root);
            if (op === 'package')
                return createReleasePackage(root, typeof a.output === 'string' ? a.output : undefined);
            return exportMatrix(root, Array.isArray(a.presets) ? a.presets.map(String) : [], String(a.mode ?? 'release'), Number(a.timeoutMs ?? 600000));
        }
        ...
        case 'session_update': return updateSession(root, a);
...
export async function doAnything(input) {
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        const step = { ...original, args: interpolateDeep(original.args ?? {}, context) };
        ...
        if (!success) {
            ...
            context.steps[id] = item;
            failed = true;
            if (input.stopOnError !== false && step.continueOnError !== true)
                break;
        }
    }
...
//# sourceMappingURL=ai-anything.js.map