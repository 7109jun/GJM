...
export function runGodot(args: string[], cwd: string, timeoutMs: number, env?: NodeJS.ProcessEnv): Promise<CommandResult> {
...
function hasGodotDiagnosticFailure(result: CommandResult): boolean {
...
export async function validateGodotProject(projectRoot: string): Promise<CommandResult> {
  ...
  if (result.code === 0 && hasGodotDiagnosticFailure(result)) return { ...result, code: 1 };
  return result;
}

export function godotOutputIndicatesFailure(result: CommandResult): boolean {
