...
import { normalizeProjectRoot } from './fs-safe.js';
...
function gv(value: JsonValue | import('./types.js').GodotTypedValue, refs?: { sub?: Map<string, number>; ext?: Map<string, number> }, godotMajor = 4): string {
  if (value && typeof value === 'object' && !Array.isArray(value) && '$type' in value) {
    const tv = value as import('./types.js').GodotTypedValue;
    const v = tv.value as any;
    ...
    switch (tv.$type) {
      ...
      case 'Transform2D': return `Transform2D(${arr.map((x: any) => Number(x) || 0).slice(0,6).join(', ')})`;
      case 'Transform3D': return `Transform3D(${arr.map((x: any) => Number(x) || 0).slice(0,12).join(', ')})`;
      case 'NodePath': return `NodePath(${q(String(tv.value ?? ''))})`;
      case 'SubResource': { const id = String(tv.value ?? ''); return godotMajor >= 4 ? `SubResource(${q(id)})` : `SubResource(${refs?.sub?.get(id) ?? (Number(id) || 1)})`; }
      case 'ExtResource': { const id = String(tv.value ?? ''); return godotMajor >= 4 ? `ExtResource(${q(id)})` : `ExtResource(${refs?.ext?.get(id) ?? (Number(id) || 1)})`; }
    }
  }
  ...
  if (Array.isArray(value)) return `[${value.map(v => gv(v as any, refs, godotMajor)).join(', ')}]`;
  return `{ ${Object.entries(value).map(([k, v]) => `${q(k)}: ${gv(v as any, refs, godotMajor)}`).join(', ')} }`;
}
function collectResources(action: MakeSceneAction): Map<string, string> {
  const paths = new Set<string>();
  for (const resource of action.resources ?? []) if (resource.path) paths.add(resource.path);
  if (action.script) paths.add(action.script);
  if (action.root?.script) paths.add(action.root.script);
  const walk = (node: MakeNodeSpec) => {
    if (node.script) paths.add(node.script);
    for (const child of node.children ?? []) walk(child);
  };
  for (const node of action.nodes ?? []) walk(node);
...
function emitNode(lines: string[], node: MakeNodeSpec, resources: Map<string, string>, subresources: Map<string, number>, godotMajor: number) {
  ...
  for (const [key, value] of Object.entries(node.properties ?? {})) lines.push(`${key} = ${gv(value, { sub: subresources, ext: new Map([...resources.values()].map((id, i) => [id, i + 1])) }, godotMajor)}`);
  for (const child of node.children ?? []) emitNode(lines, { ...child, parent: child.parent ?? node.name }, resources, subresources, godotMajor);
}
...
export function sceneText(action: MakeSceneAction, godotMajor = 4): string {
  ...
  for (const [resource, id] of resources) {
    ...
    if (godotMajor >= 4) lines.push(`[ext_resource type=\"${type}\" path=${q(resource)} id=${q(id)}]`);
    else lines.push(`[ext_resource path=${q(resource)} type=\"${type}\" id=\"${id}\"]`);
  }
  for (const sub of subresourceList) {
    ...
    for (const [key, value] of Object.entries(sub.properties ?? {})) lines.push(`${key} = ${gv(value, { sub: subresourceIds, ext: new Map([...resources.values()].map((id, i) => [id, i + 1])) }, godotMajor)}`);
  }
  ...
  for (const [key, value] of Object.entries(root.properties ?? {})) lines.push(`${key} = ${gv(value, refs, godotMajor)}`);
  for (const node of nodes) emitNode(lines, node, resources, subresourceIds, godotMajor);
...
async function mergeInputAction(root: string, name: string, deadzone: number, events: JsonValue[]) {
  const file = safeProjectPath(root, 'project.godot');
  await ensureProjectFile(root);
  const raw = await fs.readFile(file, 'utf8');
  const lines = raw.split(/\r?\n/);
  let section = lines.indexOf('[input]');
  if (section < 0) { while (lines.length && lines.at(-1) === '') lines.pop(); lines.push('', '[input]'); section = lines.length - 1; }
  let end = lines.length;
  for (let i = section + 1; i < lines.length; i++) if (/^\[[^\]]+\]$/.test(lines[i])) { end = i; break; }
  const eventText = events.length ? `Array([${events.map(e => gv(e as any)).join(', ')}])` : 'Array([])';
  const line = `${name}={\"deadzone\":${deadzone},\"events\":${eventText}}`;
  const idx = lines.findIndex((l, i) => i > section && i < end && l.startsWith(`${name}=`));
  if (idx >= 0) lines[idx] = line; else lines.splice(end, 0, line);
...
function featureNode(kind: import('./types.js').MakeFeatureKind, godotMajor: number, name: string, parent = '.', properties: Record<string, import('./types.js').JsonValue | import('./types.js').GodotTypedValue> = {}, script?: string): import('./types.js').MakeNodeSpec {
  const major4 = godotMajor >= 4;
  const base: import('./types.js').MakeNodeSpec = {
    name,
    type: 'Node',
    parent,
...
function expandFeature(action: import('./types.js').MakeFeatureAction, godotMajor: number): import('./types.js').MakeSceneAction {
  const major4 = godotMajor >= 4;
...
function coalesceFeatures(actions: import('./types.js').MakeAction[], godotMajor: number): import('./types.js').MakeAction[] {
  const expandedActions: import('./types.js').MakeAction[] = [];
  for (const action of actions) {
    if (action.type === 'template') expandedActions.push(...expandTemplates([action.name], godotMajor));
    else expandedActions.push(action);
  }
  const out: import('./types.js').MakeAction[] = [];
  const featureScenes = new Map<string, import('./types.js').MakeSceneAction>();
  const flush = (scene: string) => {
    const value = featureScenes.get(scene);
    if (value) { out.push(value); featureScenes.delete(scene); }
  };
  for (const action of expandedActions) {
    if (action.type === 'feature') {
      ...
      if (!existing) featureScenes.set(expanded.path, expanded);
      else {
        const isRootFeature = action.kind === '2d_root' || action.kind === '3d_root' || action.kind === 'hybrid_root';
        if (isRootFeature) existing.root = expanded.root;
        else existing.nodes = [...(existing.nodes ?? []), ...(expanded.nodes ?? [])];
      }
      continue;
    }
  ...
  for (const scene of [...featureScenes.keys()]) flush(scene);
  return out;
}
    ...
    switch (action.type) {
      ...
      case 'asset_base64':
        ...
        if (!dryRun) {
          ...
          if (data.length > 32 * 1024 * 1024) throw new Error('asset_base64 exceeds 32 MiB.');
      ...
      case 'input_action':
        ...
        if (!dryRun) await mergeInputAction(root, action.name, action.deadzone ?? 0.5, action.events ?? []);
        break;
      case 'autoload':
        changes.push({ type: action.type, name: action.name, path: action.path });
        if (!dryRun) await mergeProjectSetting(root, 'autoload', action.name, action.enabled === false ? '""' : action.path);
        break;
      case 'godot_script':
        ...
        if (!dryRun) {
          ...
          if (!task.ok) throw new Error(`GJM godot_script failed (exit ${task.exitCode}): ${task.stderr || task.stdout}`);
      ...
      case 'feature': {
        ...
        if (!dryRun) await writeText(root, expanded.path, sceneText(expanded, godotMajor));
        break;
      }
    }
  }
