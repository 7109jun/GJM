...
import type { BehaviorFile, CommandResult, JsonValue } from './types.js';
...
function parseMarkers(stdout: string): Array<Record<string, unknown>> {
  const out: Array<Record<string, unknown>> = [];
  for (const line of stdout.split(/\r?\n/)) {
    if (!line.startsWith('GJM_NODE ')) continue;
    try { out.push(JSON.parse(line.slice('GJM_NODE '.length))); } catch { /* ignore malformed marker */ }
  }
  return out;
}
...
export async function evaluateAssertions(
  ...
  for (let i = 0; i < assertions.length; i++) {
    ...
    switch (a.type) {
      ...
      case 'stdout_regex': {
        let ok = false;
        try { ok = new RegExp(a.pattern, 'm').test(command.stdout); } catch { /* invalid regex = false */ }
      ...
      case 'file_contains': {
        let actual = '';
        try { actual = await fs.readFile(safeProjectPath(projectRoot, a.path), 'utf8'); } catch { /* handled below */ }
      ...
      case 'node_property': {
        const found = markers.find(m => m.path === a.path || m.label === a.path);
        const actual = found?.properties && typeof found.properties === 'object' ? (found.properties as Record<string, unknown>)[a.property] : undefined;
