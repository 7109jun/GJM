...
export async function prepareFixContext(projectRoot: string, maxSnippetLines = 16) {
...
export async function applyFix(projectRoot: string, makeContent: string) {
  const startedAt = new Date().toISOString();
  const startedMs = Date.now();
  const make = validateMake(JSON.parse(makeContent) as unknown);
