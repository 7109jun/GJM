...
function textFile(pathname: string): boolean {
  return new Set(['.gd','.ts','.js','.json','.tscn','.tres','.cfg','.txt','.md','.shader','.gdshader','.cs']).has(path.extname(pathname).toLowerCase());
}

export async function buildContextPack(root: string, options: ContextPackOptions = {}) {
  ...
  async function walk(dir: string, rel = '.') {
    ...
    for (const entry of await fs.readdir(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) await walk(abs, child);
      else if (entry.isFile() && textFile(child)) {
        const stat = await fs.stat(abs);
        const item: Record<string, unknown> = { path: child.replaceAll(path.sep, '/'), size: stat.size };
        if (includeContent && stat.size <= maxFileBytes) {
          try { item.content = await fs.readFile(abs, 'utf8'); } catch {}
        }
        files.push(item);
      }
    }
