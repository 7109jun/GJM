...
async function walk(root: string, dir: string, out: string[]) {
  ...
  for (const e of entries) {
    ...
    else if (e.isFile()) out.push(path.relative(root, abs).replaceAll(path.sep, '/'));
  }
}
...
export async function getReleaseManifest(projectRoot: string) {
...
export async function createReleasePackage(projectRoot: string, requestedOutput?: string) {
  const root = normalizeProjectRoot(projectRoot);
  const outputRel = requestedOutput ?? `build/gjm-release-${Date.now()}.zip`;
  if (!outputRel.endsWith('.zip')) throw new Error('Release output must end with .zip');
...
export async function exportMatrix(projectRoot: string, presets: string[], mode: string, timeoutMs: number) {
  const root = normalizeProjectRoot(projectRoot);
  if (presets.length === 0) throw new Error('presets must contain at least one Godot export preset name.');
  const results: Array<{preset:string; output:string; code:number; stdout:string; stderr:string; timedOut?:boolean}> = [];
