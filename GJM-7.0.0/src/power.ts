...
export type PowerResult = Record<string, unknown>;
...
function lineCount(s: string) { return s.length ? s.split(/\r?\n/).length : 0; }
function countMatches(s: string, re: RegExp) { return (s.match(re) ?? []).length; }
...
function parseScene(text: string) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (n) nodes.push({ name: n[1], type: n[2] ?? 'Node', parent: n[3] ?? '' });
    const r = line.match(/^\[ext_resource\s+path="([^"]+)".*type="([^"]+)"/);
    if (r) resources.push({ path: r[1], type: r[2] });
  }
...
export async function auditProject(root: string, options: Record<string, unknown> = {}): Promise<PowerResult> {
  ...
  for (const rel of files) {
    ...
    if (stat.size > 10 * 1024 * 1024) issues.push({ severity:'warning', code:'large-file', path:rel, bytes:stat.size });
    if (TEXT_EXT.has(ext) && stat.size <= 2 * 1024 * 1024) {
      const text = await fs.readFile(p, 'utf8');
      if (ext === '.tscn') details[rel] = parseScene(text);
      if (ext === '.gd' || ext === '.gdshader') {
        const parsed = parseScript(text);
        details[rel] = parsed;
        if (parsed.delimiterBalance !== 0) issues.push({ severity:'error', code:'delimiter-balance', path:rel, delta:parsed.delimiterBalance });
        if (parsed.todo > 0) issues.push({ severity:'info', code:'todo-marker', path:rel, count:parsed.todo });
      }
      if (ext === '.godot') {
  ...
  try { project = await fs.readFile(safeProjectPath(root,'project.godot'),'utf8'); } catch {}
  return {
...
function pngRgba(width: number, height: number, pixels: Buffer) {
...
export async function generateTexture(root: string, options: Record<string, unknown> = {}): Promise<PowerResult> {
  ...
  const parse = (c: string) => { const s=c.replace('#','').padEnd(6,'0'); return [parseInt(s.slice(0,2),16),parseInt(s.slice(2,4),16),parseInt(s.slice(4,6),16),255]; };
  ...
  for (let y=0;y<height;y++) for (let x=0;x<width;x++) {
    ...
    if (kind==='noise') { r=rnd()*255; g=rnd()*255; bl=rnd()*255; }
    if (kind==='solid') { r=a[0]; g=a[1]; bl=a[2]; }
    pixels[i]=Math.round(r); pixels[i+1]=Math.round(g); pixels[i+2]=Math.round(bl); pixels[i+3]=255;
  }
  const png=pngRgba(width,height,pixels); await ensureParent(safeProjectPath(root,out)); await fs.writeFile(safeProjectPath(root,out),png);
  return {ok:true, code:'GJM-OK', path:out, width, height, kind, bytes:png.length, sha256:sha256(png)};
}

export async function editAnimation(root: string, options: Record<string, unknown>): Promise<PowerResult> {
  const file = String(options.file ?? 'animations/clip.gjm.animation.json');
  const p=safeProjectPath(root,file); const data:any=JSON.parse(await fs.readFile(p,'utf8'));
  ...
  if (!data.tracks[trackIndex]) data.tracks[trackIndex]={property:String(options.property??'position'),keys:[]};
  const keys=Array.isArray(data.tracks[trackIndex].keys)?data.tracks[trackIndex].keys:[];
  if (action==='add_key') keys.push({time:Number(options.time??0),value:options.value ?? [0,0,0]});
  else if (action==='remove_key') { const t=Number(options.time??-1); data.tracks[trackIndex].keys=keys.filter((k:any)=>Number(k.time)!==t); }
  else if (action==='move_key') { const from=Number(options.fromTime??-1); const to=Number(options.time??0); for(const k of keys) if(Number(k.time)===from) k.time=to; }
  else if (action==='set_value') { const t=Number(options.time??-1); for(const k of keys) if(Number(k.time)===t) k.value=options.value; }
  else if (action==='sort') keys.sort((a:any,b:any)=>Number(a.time)-Number(b.time));
  else throw new Error(`Unsupported animation edit: ${action}`);
  data.tracks[trackIndex].keys=keys.sort((a:any,b:any)=>Number(a.time)-Number(b.time));
  data.updatedAt=now(); await fs.writeFile(p,JSON.stringify(data,null,2)+'\n','utf8');
  return {ok:true, code:'GJM-OK', file, action, trackIndex, keyCount:data.tracks[trackIndex].keys.length, duration:Math.max(0,...data.tracks.flatMap((t:any)=>Array.isArray(t.keys)?t.keys.map((k:any)=>Number(k.time)||0):[]))};
}

export async function profileCompare(root: string, options: Record<string, unknown> = {}): Promise<PowerResult> {
  const aFile=String(options.baseline??'.gjm_profile.json'); const bFile=String(options.current??'.gjm_profile.current.json');
  const a:any=JSON.parse(await fs.readFile(safeProjectPath(root,aFile),'utf8')); const b:any=JSON.parse(await fs.readFile(safeProjectPath(root,bFile),'utf8'));
  const delta=(k:string)=>({baseline:a[k],current:b[k],delta:Number(b[k]??0)-Number(a[k]??0),percent:a[k] ? ((Number(b[k])-Number(a[k]))/Number(a[k]))*100 : null});
...
export async function shaderValidate(root: string, options: Record<string, unknown> = {}): Promise<PowerResult> {
  const shader=String(options.shader??'');
  const file=String(options.file??'');
  if (!shader && !file) throw new Error('shaderValidate requires shader or file.');
...
export async function autoplayFuzz(root: string, options: Record<string, unknown>): Promise<PowerResult> {
  const base=typeof options.behavior==='string'?options.behavior:JSON.stringify(options.behavior);
  const runs=Math.max(1,Math.min(50,Number(options.runs??10))); let seed=Number(options.seed??42)>>>0;
  const rnd=()=>{seed=(1103515245*seed+12345)>>>0;return seed/0xffffffff};
  const reports: any[]=[];
  for(let i=0;i<runs;i++){
    const b:any=JSON.parse(base);
    ...
    for(let j=0;j<count;j++) variant.push({type:'key_press',key:keys[Math.floor(rnd()*keys.length)],frames:Math.floor(rnd()*4)+1});
    b.actions=variant; b.name=`fuzz-${i+1}`;
    reports.push(await runAutoplay(root,{behavior:JSON.stringify(b),runs:1,stopOnFailure:true}));
  }
  const failed=reports.filter((r:any)=>!r.ok).length;
  return {ok:failed===0,code:failed===0?'GJM-OK':'GJM-E402',seed:rnd(),runs,failed,reports};
}

export async function inputAudit(root: string, options: Record<string, unknown> = {}): Promise<PowerResult> {
...
export async function releaseReadiness(root: string): Promise<PowerResult> {
  const audit=await auditProject(root,{maxFiles:10000});
  const checks:{name:string,ok:boolean,required:boolean,detail:unknown}[]=[];
  checks.push({name:'project.godot',ok:Boolean(audit.projectPresent),required:true,detail:audit.projectPresent});
  checks.push({name:'no_static_errors',ok:Boolean(audit.ok),required:true,detail:(audit.issues as unknown[]).filter((x:any)=>x.severity==='error')});
  try { const git=await fs.access(safeProjectPath(root,'.git')); checks.push({name:'git',ok:true,required:false,detail:Boolean(git)}); } catch { checks.push({name:'git',ok:false,required:false,detail:'not a git working tree'}); }
  let size=0; for(const rel of await walk(root)){ try{size+=(await fs.stat(safeProjectPath(root,rel))).size;}catch{} }
